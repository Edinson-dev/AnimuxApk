...
export default function ChannelCard({ channel, onPlay, isFavorite, onToggleFavorite }) {
  ...
  return (
    ...
    >
      <div className={`relative overflow-hidden rounded-xl bg-[#0a0a0a] border border-white/5 transition-all duration-500 group-hover:border-blue-500/50 group-hover:shadow-[0_0_40px_rgba(59,130,246,0.3)] ${isVOD ? 'aspect-[2/3]' : 'aspect-video'}`}>
        <img 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center backdrop-blur-[2px]">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-2xl scale-50 group-hover:scale-100 transition-transform duration-500 shadow-blue-600/50">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          <span className={`px-2 py-0.5 backdrop-blur-md border border-white/10 rounded text-[8px] font-black text-white uppercase tracking-widest shadow-lg ${isVOD ? 'bg-black/60' : 'bg-blue-600/80 animate-pulse'}`}>
            {badgeText}
          </span>
          {channel.isNew && (
        ...
        {onToggleFavorite && (
          <button 
            ...
            className={`absolute top-3 right-3 p-2 rounded-full backdrop-blur-md border transition-all ${isFavorite ? 'bg-blue-600 border-blue-600 text-white' : 'bg-black/40 border-white/10 text-white/70 hover:text-white hover:bg-blue-600/50 hover:border-blue-500/50'}`}
          >
      ...
      <div className="px-1 py-1">
        <h4 className="text-[11px] md:text-sm font-black text-white/90 truncate uppercase tracking-tight group-hover:text-blue-400 transition-colors">
